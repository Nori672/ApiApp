...
//ApiFragmentクラスで実際のAPI通信や取得したJsonデータをApiResponseへの変換を行う
class ApiFragment:Fragment() {
    private val apiAdapter by lazy { ApiAdapter(requireContext()) }
    private  val handler = Handler(Looper.getMainLooper()) //描画はMainスレッドで行い、API通信の処理は別スレッドで行う必要があるためHandlerを使用する。

    private var fragmentCallBack: FragmentCallBack? = null // Fragment -> Activity にFavoriteの変更を通知する

    private var page = 0 //最初のページを0とするページを表現する変数
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        //SwipeRefreshLayoutでリフレッシュ動作を行った時に呼び出されるリスナーをセット
    ...
    //このメソッド内でAPI通信を行いデータを取得する
    private fun updateData(isAdd : Boolean = false){ //引数が空のときは、falseが勝手に入る。
        if (isLoading) {
            return
        } else {
            isLoading = true
        ...
        //ベースとなるURLにGETパラメータというものを追加して、取得するデータの条件を決める
        ...
        //ここで実際にHttp通信を行う
        //enqueueの引数にCallbackを設定し、Callback内で通信成功時と失敗時の処理を書く
        client.newCall(request).enqueue(object : Callback{
            override fun onFailure(call: Call, e: IOException) { // Error時の処理（ onFailureメソッド）
                //RecycleViewを空のリストで更新
                e.printStackTrace()
                handler.post {
            ...
            override fun onResponse(call: Call, response: Response) { // 成功時の処理（onResponseメソッド）
                //取得したデータ(Json)をApiResponseに変換し、RecycleViewを更新
                var list = listOf<Shop>()
                response.body?.string()?.also {
                    val apiResponse = Gson().fromJson(it, ApiResponse::class.java) //ここでJsonデータから、ApiResponse（javaクラス）のオブジェクトに変換
    ...
    private fun updateRecyclerView(list: List<Shop>, isAdd: Boolean){
        if (isAdd) {
            ...
            apiAdapter.refresh(list) //API通信により取得したlistをRecyclerViewに反映させるためApiAdapterのrefreshメソッドを呼び出す
        }
        swipeRefreshLayout.isRefreshing = false // リフレッシュ動作が終了したらSwipeRefreshLayoutのくるくるを非表示にするため、falseにする
    }
}